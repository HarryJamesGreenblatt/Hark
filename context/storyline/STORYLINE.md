...
- **HARK 2.1.0 — the locked backlog (Episode 37; item #1 shipped in Episode 38):** a planning session
  compiled + **locked** the next milestone's scope. **#1 rebrand to HARK = Hear · Adapt · Render · Keep is
  DONE** (Episode 38, `70aed5a` — full re-story, transcription folded into Hear, Render = Vision). The
  remaining five, each grounded to a code seam: **(2)** **organic eye motion** for the Vision eye
  ...
  **installer's long pre-UAC startup delay** (splash for the post-UAC gap + **measure first** the pre-UAC
  single-file verify/extract cost; *not* reverting to WinForms). Suggested order: the **export-polish
  cluster** (#3·#4·#5, all in `Hark.App/Reporting/`) next, then the installer delay, with the eye-motion
  spike in parallel. Full list + seams: `/memories/repo/hark-2.1.0-backlog.md`. Records:
