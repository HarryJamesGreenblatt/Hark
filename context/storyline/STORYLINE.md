...
_Last updated: 2026-09-01 (end of Episode 32. An evidence check on a captured Aladdin session: the **topics** track the transcript faithfully, but the **image↔topic** match was loose *by architecture* (the scene and diagram are independent distillations of the same window). Fixes: **chained the scene to the diagram's topic** (`topicAnchor` — diagram runs first, its title anchors the scene) and **realigned the prompt composer to FLUX's first-party grammar** (front-loaded subject, positive-only, negatives purged, ~40-80 words) after corroborating BFL's guide. Also: a pupil **"ether" alpha** (0.85, red glow bleeds through) + slower standby fade, and **temp-cache hygiene** (scene cache now cleared on toggle-off + app exit). Phase 2 (**FLUX JSON structured prompt**) is teed up. See [`EP32`](./EP32-topic-chained-scenes-and-flux-idiomatic-composer.md))._
...
- **Vision image↔topic alignment — chained + FLUX-realigned, awaiting a live read (Episode 32):** the scene
  ...
  **FLUX-idiomatic** (front-loaded, positive-only, negatives purged, ~40-80 words). **Open:** a live A/B — do
  scenes now visibly illustrate the titled topic, and do the FLUX prompts read cleaner? **Phase 2 (teed up):**
  emit the `VisualConcept` as **FLUX JSON** (BFL "maximum control"; schema maps ~1:1) + optional **camera/lens**
  specificity — FLUX-specific, needs a provider split (gpt-image keeps prose). Do only if the natural-language
  composer isn't tight enough. Full record: [`EP32`](./EP32-topic-chained-scenes-and-flux-idiomatic-composer.md).
