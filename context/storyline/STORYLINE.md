...
- **HARK 2.1.0 — the locked backlog (Episode 37):** a planning session compiled + **locked** the next
  milestone's scope (no code yet). Six items, each grounded to a code seam: **(1)** rebrand to **HARK =
  Hear · Adapt · Render · Keep** (a code+docs scrub); **(2)** **organic eye motion** for the Vision eye
  (gaze/look-at, a new audio band, attraction/repulsion — a research spike, the 2D-gaze worry answered by
  **layered pupil-offset**); **(3)** a **generated session title** (replace the hardcoded
  `"Hark session report"` in `OverlayWindow.BuildSessionReport()` — one seam fans out to all five formats);
  **(4)** **PDF export in light mode** (fix WebView2's white page-border clash via a `lightMode` flag on
  `HtmlReportWriter`); **(5)** a **consistent HAL icon across exports** (embed `Hark.App/Assets/Icon.png`
  everywhere — HTML/PDF use a CSS faux-eye today, MD/Word omit it, PPTX uses the scene); **(6)** the
  **installer's long pre-UAC startup delay** (splash for the post-UAC gap + **measure first** the pre-UAC
  single-file verify/extract cost; *not* reverting to WinForms). Suggested order: rebrand first, then the
  export-polish cluster (all in `Hark.App/Reporting/`), then the installer delay, with the eye-motion spike
  in parallel. Full list + seams: `/memories/repo/hark-2.1.0-backlog.md`. Record:
  [`EP37`](./EP37-2.1.0-backlog-lock.md).
