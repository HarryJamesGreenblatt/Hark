...
_Last updated: 2026-09-03 (end of Episode 43. **Re-cut HARK 2.1.0** to carry a headset fix, and prepped a **Microsoft Hackathon** entry. Built reproducible **animated Oracle-eye thumbnails** (`assets/thumbnail/`) — a hero tile where the Oracle *reads its own name into being*: Hear (fixate) · Adapt (gaze-scan) · Render (vision into the pupil) · Keep (shutter-blink), with H·A·R·K acronym emphasis — plus a pure-eye fallback. Then a live cross-machine test surfaced a **headset-only reactivity** gap (mic RMS ≪ system playback), fixed by giving the **mic and system audio independent reactivity paths** (measured pre-mix, combined by `max()`, mic ~2.4× hotter with its own noise gate) — validated live (headset-only lights the eye; mixes cleanly with AI TTS narration). `v2.1.0` was **deleted and re-cut** at the fixed HEAD. `dotnet test` 4 passed. See [`EP43`](./EP43-hackathon-thumbnails-and-per-source-reactivity.md). Prior: Episode 42 cut 2.1.0 with the research-grounded eye-gaze.)_
