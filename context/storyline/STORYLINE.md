...
_Last updated: 2026-09-01 (end of Episode 33. Added the OpenXML **Word (.docx)** report writer — transcript + recaps + the vision slideshow with **inline-embedded scene images** — so the Save picker now offers **Markdown · Word · Web page**; backed by a real **`Hark.Tests` xUnit project** (added to the solution) that validates the .docx with the Open XML validator (**0 errors**) after a course-correction to stop trusting a throwaway reflection harness and actually test the source. **Next front (deferred):** report **formatting** across all types — they dump plainly and images land on the next page instead of beside the beat they illustrate. PPTX (the flagship) is still last; Phase 2 FLUX JSON tabled. See [`EP33`](./EP33-word-export-and-testing-the-source.md))._
...
- **Report formatting — the next front, ALL types (Episode 33):** the MD/HTML/Word reports **dump content
  plainly**, and in **Word the scene images land on the next page** rather than beside the beat they
  illustrate. Wants real layout: keep each scene **with its beat** (keep-together / two-column beat cards),
  consistent heading styles, spacing, and image sizing — applied across **MD / HTML / Word** and carried into
  **PPTX**. Also still to build: **PPTX** (the flagship — one slide per beat, OpenXML) and **PDF** (WebView2 on
  the HTML). Full record: [`EP33`](./EP33-word-export-and-testing-the-source.md).
