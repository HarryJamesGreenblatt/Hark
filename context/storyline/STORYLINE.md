...
_Last updated: 2026-09-04 (end of Episode 44. Designed the **2-minute Hackathon submission video** as a **self-demonstrating run-through** — the soundtrack *is* the input: play **Bing Crosby's "Do You Hear What I Hear?"** → HARK hears/captions it, renders Vision beats, and exports a report. The song's **HEAR / SEE / KNOW** refrains map onto **Hear / Render / Adapt-Keep**, it transcribes cleanly (crooned, near-spoken — sung+music breaks ASR, so "Every Breath You Take" was dropped), and its shifting imagery yields varied beats. Full brief in [`../hackathon-video-plan.md`](../hackathon-video-plan.md) for production on another machine. Prior (EP43): re-cut 2.1.0 with the headset-reactivity + summarizer-named-speaker fixes, animated thumbnails, and the hackathon submission copy.)_
...
| 42 | 2026-09-03 | [The Oracle Learns to Look: Grounded Eye-Gaze & Cutting 2.1.0](./EP42-oracle-gaze-research-and-2.1.0.md) | Closed the last 2.1.0 item (#2, **organic eye motion**) — and **cut HARK 2.1.0**. After a first code pass was **reverted** for skipping research, did a proper literature pass (browser/DDG → new artifact `context/research/eye-motion-gaze.md`, with the **"Eyes Alive"** statistical model extracted from the paper) and built a grounded **2D iris-offset gaze**: min-jerk saccades on the **main sequence** `T=α+βA`, Eyes-Alive magnitude/direction sampling, ~10% undershoot + corrective, and **1/f (pink-noise)** fixation jitter, wrapping the cornea+pupil+sheen in an `EyeGazeGroup` (silver frame/socket stay fixed; gloss decoupled). Two modes: **pill-hover** → quick-but-gentle cursor pursuit; **ambient** → sound-reactive (audio-gated glances + level-scaled jitter, no autonomous timer). Added an **intra-pupil parallax** so the bright HAL-like core *leads* the red disc (`pupilLead≈0.75`) — the gaze reads as aimed from the pupil point. Corrected the design live (no socket clip — it cut the glow bloom; the offset clamp suffices). `dotnet test` 4 passed; user verdict *"looking pretty dang good."* Commits `23f727d..a9ef78a`, tag **`v2.1.0`**. |
| 43 | 2026-09-03 | [Hackathon Polish: Animated Thumbnails & Per-Source Eye Reactivity (2.1.0 re-cut)](./EP43-hackathon-thumbnails-and-per-source-reactivity.md) | Prepped a **Microsoft Hackathon** entry and **re-cut HARK 2.1.0** to carry a headset fix. Built reproducible **Python (Pillow/numpy)** generators for **animated Oracle-eye thumbnails** (`assets/thumbnail/`): a hero tile where the Oracle *reads its own name into being* — **Hear** (fixate) · **Adapt** (a three-point gaze **scan**, not pills — too busy at tile size) · **Render** (a vision renders into the pupil) · **Keep** (a **shutter-blink** = camera snapshot) — with **H·A·R·K** acronym emphasis; ~2 MB seamless loops, plus a pure-eye fallback (research: GIF is the tile mechanism, ≤5 MB, verify it doesn't flatten on upload). Then a live cross-machine test found a **headset-only reactivity** gap (mic RMS ≪ system playback, and level came from the *mixed* stream); fixed per the user's steer — not by boosting the mic **input** (feedback/noise) but by giving the **mic and system audio their own reactivity paths**: `AudioFeatures` carries both bands, `HarkSession` measures each on its own windowed RMS (mic **pre-mix**), and the eye is driven by **`max()`** of two independently-tuned gains (mic **~2.4× hotter** + its own noise gate). Validated live (headset-only lights the eye; HARK + a Bing-AI Navy TTS narration each keep separate reactivity). `dotnet test` 4 passed. `v2.1.0` **deleted + re-cut** at the fixed HEAD. Commits `c21ed01` (thumbnails) · `a66a0aa` (reactivity). |
| 44 | 2026-09-04 | [The Recursive Demo: A Self-Demonstrating Hackathon Video](./EP44-hackathon-video-recursive-demo.md) | Designed the **2-minute Hackathon video** as a **self-demonstrating run-through** — the soundtrack *is* HARK's input: play **Bing Crosby's "Do You Hear What I Hear?"** and HARK hears/captions it, renders Vision beats, and exports a report. The song was chosen because it **transcribes cleanly** (crooned/near-spoken — sung vocals + music defeat ASR, so The Police's "Every Breath You Take" was tested and dropped ~every other word), its title leads with **HEAR**, and its refrains **HEAR / SEE / KNOW** map onto **Hear / Render / Adapt-Keep** (the song asks the three questions HARK answers), with shifting imagery giving *varied* Vision beats. A real capture produced **8 varied beats**, a peace-themed summary, the speaker named **"Bing Crosby,"** and a clean lyric transcript. Latency hidden by **recording the instant timeline replay** of pre-cached beats (alt: speed-ramp with continuous song audio). Full production brief: [`context/hackathon-video-plan.md`](../hackathon-video-plan.md). No code changed. |
