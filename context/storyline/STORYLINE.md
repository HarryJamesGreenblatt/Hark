...
_Last updated: 2026-09-03 (end of Episode 42. **Cut HARK 2.1.0.** Closed the final 2.1.0 item — **organic eye motion** — with a research-grounded 2D iris-offset gaze on the Vision eye: min-jerk saccades on the main sequence, "Eyes Alive" magnitude/direction statistics, ~10% undershoot + corrective, and 1/f pink-noise fixation jitter, in two modes (mind-map **pill-hover** cursor pursuit vs a **sound-reactive** ambient that jitters when stimulated and rests when quiet), plus an **intra-pupil parallax** so the bright HAL-like core leads the red disc. A first code pass was **reverted** for skipping research; the grounding lives in [`../research/eye-motion-gaze.md`](../research/eye-motion-gaze.md). `dotnet test` 4 passed. See [`EP42`](./EP42-oracle-gaze-research-and-2.1.0.md). Prior: Episode 41 diagnosed + deferred the installer UAC delay and gated the provisioning form.)_
