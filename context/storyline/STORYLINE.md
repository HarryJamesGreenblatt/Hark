...
_Last updated: 2026-09-01 (end of Episode 31. Closed EP30's **null-scene gap**: the crystal-ball pupil now **cross-fades to the red sound-reactive glow** on a live topic shift or a failed/again-frequent FLUX render (`FadePupilToEye`), and fades the next scene back in when ready — no spinner, no stale-image shuffle. The idle filler split cleanly into **LIVE image-blink** (cycle recent images, keep the topic) vs **REVIEW topic-slideshow** (synchronized topic+scene, ~7 s cadence); the topic-walk is review-only so it never drags the mind-map backward mid-live. A formal **state-machine refactor (Nystrom)** was mapped (concurrent Topic + Pupil machines) but **deliberately deferred** — the informal handling now works. See [`EP31`](./EP31-resting-eye-null-scene-and-deferred-state-machine.md))._
