...
_Last updated: 2026-09-02 (end of Episode 34. Redesigned the session report around a **shared "beat card" layout** — designed first in **HTML** (a small design system: HAL-eye hero, styled section heads, and four reusable cards — topic, speaker, **beat**, transcript), then carried verbatim into **Markdown** and **Word**. **Vision now leads** the report (then Conversation summary, Speakers, Transcript), and each beat keeps its **scene beside its mind-map** — in Word a **keep-together two-column table** that fixes the images-land-on-the-next-page bug. Also fixed that **recaps were missing** from saves (a new `ReportRecapsRequested` event generates both scopes on demand) and added two **Vision QoL** touches: **save-progress feedback** (busy button + off-UI-thread write) and the review slideshow **holding while you hover a mind-map pill**. **Next front:** **PPTX** (the flagship — one slide per beat) then **PDF via WebView2**. See [`EP34`](./EP34-report-layout-and-vision-qol.md))._
...
- **Report formatting — RESOLVED (Episode 34):** the reports no longer dump plainly. A **layout language** was
  ...
  **Vision leads** every format. In **Word** the beat is a **keep-together two-column table** (`CantSplit`),
  ...
  so summaries always appear. **Remaining, in order:** **PPTX** (the flagship — one slide per beat, Open XML,
  inheriting the same card grammar) and **PDF** (WebView2 on the styled HTML). Full record:
  [`EP34`](./EP34-report-layout-and-vision-qol.md).
